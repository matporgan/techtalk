	...
	<div class="row center">
		<ul class="pagination">
			@if($orgs->lastPage() == 1)
				...
				<li class="active"><a href="{{ $orgs->currentPage() . $search }}">1</a></li>
				<li class="disabled"><i class="material-icons">chevron_right</i></li>
			@else
				<li class=@if($orgs->currentPage() == 1)"disabled"@endif>
					<a href="{{ $orgs->previousPageUrl() . $search }}" class="page"><i class="material-icons">chevron_left</i></a>
				</li>
				@for($i = 1; $i <= $orgs->lastPage(); $i++)
					<li class=@if($orgs->currentPage() == $i)"active"@else"waves-effect"@endif>
						<a href="{{ $orgs->url($i) . $search  }}" class="page">{{ $i }}</a>
					</li>
				@endfor
				<li class=@if($orgs->currentPage() == $orgs->lastPage())"disabled"@endif>
					<a href="{{ $orgs->nextPageUrl() . $search  }}" class="page"><i class="material-icons">chevron_right</i></a>
				</li>
			@endif
		</ul>
	</div>
